...
export const fetchVisitType = async (userDetails) => { // Pass userDetails as a parameter
    try {
        axios.defaults.headers.common['Authorization'] = `Bearer ${userDetails?.[0]?.accessToken}`;
        const response = await axios.get('/api/user/visit/type');

        console.log('visitType: ', response);
